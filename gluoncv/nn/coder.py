...
class NumPyNormalizedBoxCenterEncoder(object):
    ...
    def __call__(self, samples, matches, anchors, refs):
        ...
        tmp_t2 = g[2] / a[2]
        tmp_t3 = g[3] / a[3]
        tmp_t2 = np.where(tmp_t2 < 1e-20, 1e-20, tmp_t2)
        tmp_t3 = np.where(tmp_t3 < 1e-20, 1e-20, tmp_t3)
        t2 = (np.log(tmp_t2) - self._means[2]) / self._stds[2]
        t3 = (np.log(tmp_t3) - self._means[3]) / self._stds[3]
